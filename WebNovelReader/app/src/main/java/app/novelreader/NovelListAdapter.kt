...
sealed class NovelListItem {
    data class Header(val folder: NovelFolder?) : NovelListItem() // folder == null は「未分類」
...
class NovelListAdapter(private val context: Context) : BaseAdapter() {
    ...
    fun submit(novels: List<Novel>, folders: List<NovelFolder>, episodeCounts: Map<String, Int>) {
        ...
        for (folder in folders) {
            ...
            grouped.add(NovelListItem.Header(folder))
        ...
        if (unassigned.isNotEmpty()) {
            if (folders.isNotEmpty()) grouped.add(NovelListItem.Header(null))
    ...
    override fun getView(position: Int, convertView: View?, parent: ViewGroup?): View {
        return when (val item = items[position]) {
            is NovelListItem.Header -> {
                ...
                view.findViewById<TextView>(R.id.textHeader).text = item.folder?.name ?: "未分類"
                view
            }
